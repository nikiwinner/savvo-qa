 ...
 *  - Flipping manual → bank changes the link's canonical_source server-side
 *    (canonical_source still drives analytics dedup).
...
// Use a fixed past date that is guaranteed to be inside the current month.
const NOW = new Date()
...
test.describe('Canonical source toggle', () => {
  test('toggling canonical_source from manual to bank persists server-side', async ({
    page,
    loggedInPage,
  }) => {
    ...
    // Server-truth: re-query the reconciliation links API. canonical_source
    // now reflects the bank side.
    const links = await api.listReconciliationLinks(hh.id)
    const after = links.results.find((l) => l.id === link.id)
    expect(after).toBeDefined()
    expect(after!.canonical_source).toBe('bank')
  })
